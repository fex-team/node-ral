...
StringConverter.prototype.unpack = function (context) {
    return new StringDecodeStream(context.encoding);
};

StringConverter.prototype.pack = function (context, data) {
    data = data || '';
    var buffer, rs = new Readable();
    try{
        buffer = iconv.encode(data.toString(), context.encoding);
        rs._read = function(){
            rs.push(buffer);
            rs.push(null);
        };
