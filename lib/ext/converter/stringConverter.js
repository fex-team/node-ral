...
StringConverter.prototype.unpack = function (convertContext) {
    return new StringDecodeStream(convertContext.encoding);
};
...
StringConverter.prototype.pack = function (convertContext, data) {
    ...
    try{
        buffer = iconv.encode(data.toString(), convertContext.encoding);
        rs._read = function(){
            rs.push(buffer);
            rs.push(null);
        };
