...
HttpProtocol.prototype._request = function(options){
    ...
    var validPostPayload = true;
    if (options.method.toLowerCase() === 'post' && options.payload){
        if (options.payload.constructor === formData){
            opt.headers = opt.headers || {};
            opt.headers['Content-Type'] =  "multipart/form-data;boundary=" + options.payload.getBoundary();
        }else{
            opt.headers = opt.headers || {};
...
function HttpProtocolContext(serviceID, service){
    this._optNames = ['path', 'method', 'headers', 'query', 'encoding'];
